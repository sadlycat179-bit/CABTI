...
(async () => {
  ...
  for (const [type, content] of Object.entries(expected)) {
    ...
    if (type === "DRINK" && actual.firstImage !== "assets/images/cats/gallery/yangleduo-new.jpg") throw new Error(`DRINK image mismatch: ${actual.firstImage}`);
    ...
    if (type === "LAMP") await page.screenshot({ path: "tests/browser/content-lamp-mobile.png", fullPage: true });
    if (type === "DRINK") await page.screenshot({ path: "tests/browser/content-drink-mobile.png", fullPage: true });
  }
  ...
  if (daiyuResult.image !== "assets/images/cats/stories/daiyu-fight.jpg") throw new Error(`Daiyu result image mismatch: ${daiyuResult.image}`);
  if (!daiyuResult.previousText.endsWith("然后挨打。")) throw new Error("Daiyu image is not placed after the requested sentence");
  await page.screenshot({ path: "tests/browser/content-daiyu-mobile.png", fullPage: true });
  ...
  const expectedDanhuangImages = [
    { image: "assets/images/cats/stories/danhuang-schedule.jpg", ending: "曾经一度被人以为是公猫。" },
    { image: "assets/images/cats/stories/danhuang-banner.jpg", ending: "它会不会变成笑笑二世？" },
  ];
  ...
  await page.screenshot({ path: "tests/browser/content-danhuang-mobile.png", fullPage: true });
  ...
  const dialogImage = await page.locator('.dialog-story-image[src="assets/images/cats/stories/daiyu-fight.jpg"]').count();
  ...
  const danhuangDialogImages = await page.locator('.dialog-story-image[src^="assets/images/cats/stories/danhuang-"]').count();
